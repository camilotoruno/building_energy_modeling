...
def filter_cities(data, keep_cities=[], exclude_cities=[]):
    # note this modifies the original data object 
    print('\nfiltering cities')

    if exclude_cities != []:
        rows =  ~data['in.city'].isin(exclude_cities)
        data = data[rows ]
        print('\tremoving ', sum(~rows), ' exclude cities houses')
        
    if keep_cities != []:
        rows = ibuildstock['in.city'].isin(keep_cities)
        print('\tremoving not keep list ', sum(~rows), ' houses')
        data = data[rows]   
...
def create_income_bins(data):  
    # count data
    income_bins = data.value_counts("in.income")  # Replace with the column to count
...
ifolder = "/Users/camilotoruno/anaconda3/envs/research/research_data/"
ifile = "baseline_metadata_only.csv"
keep_cities = []
exclude_cities = []
acceptable_federal_poverty_levels = ['0-100%', '100-150%', '150-200%']
min_city_size = 0
city_size_limit = 400
ibuildstock = pd.read_csv(ifolder + ifile)
# optional inputs - default are empty lists
keep_cities = [
                "AZ, Phoenix",
                "CA, Los Angeles",
                "CO, Denver",
...
obuildstock = filter_poverty(obuildstock, acceptable_federal_poverty_levels)
...
if input('save output data? (note the file may be very large) [y/n]\n') == 'y':
    obuildstock.to_csv(ifolder + 'filtered_24.02.11' + ifile, index=False)
